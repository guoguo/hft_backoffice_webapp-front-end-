(function () {
    ...
    angular
        .module('app')
        .controller('BankArtificialController', BankArtificialController);

    function BankArtificialController($scope,MyTaskService,BankDepositService,Util) {
		$scope.params = {
			mytask:[],
			dialogMes:"",
			symbol:0,
		...
		$scope.startClick = function(){
			var starttime = $("#starttimeart").val().replace("年","-").replace("月","-").replace("日","");
			var endtime = $("#endtimeart").val().replace("年","-").replace("月","-").replace("日","");
