(function () {
    ...
    angular
        .module('app')
        .controller('CoinDepositController', CoinDepositController);

    function CoinDepositController($scope,MyTaskService,BankDepositService,AuditService,Util) {
		$scope.params = {
			mytasks:[],
			dialogMes:"",
			symbol:0,
		...
		function queryCoinBankTaskDetail(params){
			if($scope.params.iscurr){
				//查询数字货币充提任务
		...
		$scope.queryTask = function(e,show_user_status){
			...
			$(target).addClass('bk-t-msaCh');
			$(target).siblings().removeClass('bk-t-msaCh');
			if(show_user_status!=-2){
				delete $scope.params.iscurr;
				var mytaskArr = [];
				...
				$scope.params.mytasks = mytaskArr;
			}else{
				$scope.params.iscurr = true;
				$scope.params.mytasks = $scope.mytaskMiddle;
			}
		};
			...
			if($scope.params.symbol==9){
				MyTaskService.rejectFun($scope.rejectData).then((response) => {
					if(response.success){
						...
						$scope.params.mytasks[index].assign_role_id = $scope.symbol;
						$scope.params.mytasks[index].show_user_status=-1;
					}else{
			...
			//提交
				...
				MyTaskService.goCoinUpstairs($scope.goUpdtariParam).then((response) => {
					if(response.success){
						var index = $scope.params.index;
						Util.dialog("提交上级成功");
						$scope.params.mytasks[index].assign_role_id = $scope.params.mytasks[index].assign_role_id+1;
						if($scope.symbol==3){
							$scope.params.mytasks[index].assign_role_id = 3;
							$scope.params.mytasks[index].system_status=3;
							$scope.params.mytasks[index].show_user_status=0;
						}
